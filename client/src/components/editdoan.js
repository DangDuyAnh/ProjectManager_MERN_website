...
export default class CreateProject extends Component {
    ...
    componentDidMount() {
        axios.get(process.env.REACT_APP_API_URL + 'datk/'+this.props.match.params.id)
            .then(response => {
            this.setState({
                tendoan : response.data.tendoan,
                linkdoan : response.data.linkdoan,
                tensinhvien : response.data.tensinhvien,
                emailsinhvien : response.data.emailsinhvien,
                tengiangvien : response.data.tengiangvien,
                emailgiangvien : response.data.emailgiangvien,
...
onClick = e => {
        ...
        axios.post(process.env.REACT_APP_API_URL + 'datk/update/' + this.props.match.params.id, doan)
...
render() {
    ...
    <div>
      ...
      <h3>Update your project</h3>
        ...
        <div className="test">
        <button type="submit" onClick = {this.onClick}>Update project</button>
        </div>
    </div>
    }</div>
    )
